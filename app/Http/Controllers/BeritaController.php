...
class BeritaController extends Controller
{
    ...
    public function publik(Request $request)
    {
        ...
        $upcomingEvents = Event::where('end_date', '>=', now()->startOfDay())
            ->orderBy('end_date', 'asc') // Urutkan berdasarkan tanggal event
    ...
    public function detail(Request $request, $id)
    {
        ...
        // Ambil event mendatang (future events) yang tanggal_event > sekarang
        $upcomingEvents = Event::where('end_date', '>=', now()->startOfDay()) // Menyertakan hari ini
            ->orderBy('end_date', 'asc') // Urutkan berdasarkan tanggal event
            ->take(4) // Ambil 4 event mendatang
            ->get();
